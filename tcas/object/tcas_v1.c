 ...
 __attribute__((always_inline)) inline bool Non_Crossing_Biased_Climb()
{
    ...
    if (upward_preferred)
    {
        result = !(Own_Below_Threat()) || ((Own_Below_Threat()) && (!(Down_Separation > ALIM()))); /* operator mutation */
    }
    else
    {	
...
char *argv[];
{
    ...
    if (Cur_Vertical_Sep == 958 && High_Confidence == 1 && Two_of_Three_Reports_Valid == 1
            && Own_Tracked_Alt == 2597 && Own_Tracked_Alt_Rate == 574 
            && Other_Tracked_Alt == 4253 && Alt_Layer_Value == 0 && Up_Separation == 399
            && Down_Separation == 400 && Other_RAC == 0 && Other_Capability == 0 && Climb_Inhibit == 1)
        klee_assert(ret==0);
    exit(0);
}
