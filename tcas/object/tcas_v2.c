...
{
    int result = (Climb_Inhibit ? Up_Separation + MINSEP /* operand mutation NOZCROSS */ : Up_Separation); return result;
}
...
char *argv[];
{
    ...
    if (Cur_Vertical_Sep == 832 && High_Confidence == 1 && Two_of_Three_Reports_Valid == 1 
            && Own_Tracked_Alt == 4454 && Own_Tracked_Alt_Rate == 148 && Other_Tracked_Alt == 4683
            && Alt_Layer_Value == 3 && Up_Separation == 501 && Down_Separation == 640 
            && Other_RAC == 0 && Other_Capability == 1 && Climb_Inhibit == 1) {
        klee_assert(ret == 0); 
    }
