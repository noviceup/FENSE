 ...
 __attribute__((always_inline)) inline bool Non_Crossing_Biased_Climb()
{
    ...
    upward_preferred = (Up_Separation + NOZCROSS) > Down_Separation; // mutate
    if (upward_preferred)
    {
...
char *argv[];
{
    ...
    if (Cur_Vertical_Sep == 628 && High_Confidence == 1 && Two_of_Three_Reports_Valid == 0
            && Own_Tracked_Alt == 2784 && Own_Tracked_Alt_Rate == 4 && Other_Tracked_Alt == 3034
            && Alt_Layer_Value == 3 && Up_Separation == 399 && Down_Separation == 399
