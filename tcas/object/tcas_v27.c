 ...
 __attribute__((always_inline)) inline int alt_sep_test()
{
    ...
    int alt_sep;
    bool temp = High_Confidence && (Own_Tracked_Alt_Rate <= OLEV) ;
    enabled = temp;
...
char *argv[];
{
    ...
    if (Cur_Vertical_Sep == 592 && High_Confidence == 1 && Two_of_Three_Reports_Valid == 0 
            && Own_Tracked_Alt == 1045 && Own_Tracked_Alt_Rate == 226 && Other_Tracked_Alt == 4721
            && Alt_Layer_Value == 2 && Up_Separation == 640 && Down_Separation == 401
