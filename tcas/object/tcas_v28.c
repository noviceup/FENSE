...
{
    int result = ((Climb_Inhibit == 0) ? Up_Separation + NOZCROSS : Up_Separation); return result;
}
...
char *argv[];
{
    ...
    if (Cur_Vertical_Sep == 653 && High_Confidence == 1 && Two_of_Three_Reports_Valid == 0
            && Own_Tracked_Alt == 432 && Own_Tracked_Alt_Rate == 67 && Other_Tracked_Alt == 203
            && Alt_Layer_Value == 0 && Up_Separation == 401 && Down_Separation == 401
            && Other_RAC == 1 && Other_Capability == 0 && Climb_Inhibit == 0)
        klee_assert(ret==2);
