 ...
 __attribute__((always_inline)) inline int alt_sep_test()
{
    ...
    enabled = enabled && (Own_Tracked_Alt_Rate <= OLEV); /* && (Cur_Vertical_Sep > MAXALTDIFF); missing code */
...
char *argv[];
{
    ...
    if (Cur_Vertical_Sep == 571 && High_Confidence == 1 && Two_of_Three_Reports_Valid == 0
            && Own_Tracked_Alt == 397 && Own_Tracked_Alt_Rate == 173 && Other_Tracked_Alt == 2247
            && Alt_Layer_Value == 2 && Up_Separation == 641 && Down_Separation == 499 
            && Other_RAC == 1 && Other_Capability == 0 && Climb_Inhibit == 1)
        klee_assert(ret == 0);
