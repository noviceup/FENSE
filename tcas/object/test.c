...
int Non_Crossing_Biased_Climb, Non_Crossing_Biased_Descend, Own_Below_Threat, Own_Above_Threat;
 __attribute__((always_inline)) inline int alt_sep_test()
{
    ...
    if (enabled) { 
        if (((tcas_equipped && intent_not_known) || !tcas_equipped))
    {
...
char *argv[];
{
    ...
    if (Cur_Vertical_Sep == 958 && High_Confidence == 1 && Two_of_Three_Reports_Valid == 1
            && Own_Tracked_Alt == 2597 && Own_Tracked_Alt_Rate == 574 && Other_Tracked_Alt == 4253
            && Alt_Layer_Value == 0 && Up_Separation == 399 && Down_Separation == 400
